  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center">
      <div className={`${theme === 'dark' ? 'bg-black border border-slate-700' : 'bg-white'} w-[70%] sm:w-[50%] xl:w-[30%] p-6 rounded-lg shadow-lg relative max-h-80 overflow-y-auto`}>
        ...
        <h2 className="text-2xl font-semibold text-center mb-4">Followers</h2>
        ...
        {authUser?.followers?.length > 0 ? (
          <ul className="mt-2 space-y-3">
            {authUser.followers.map((follower) => (
              ...
              >
                <img
                  src={follower.profileImg || "/avatar.png"}
          ...
          <p className="text-center text-gray-600 mt-2">No followers yet.</p>
        )}
      </div>
    </div>
  );
